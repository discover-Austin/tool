...
class BlueprintControlTutorialTest {
    ...
    @Test
    fun `joystick tutorial steps spotlight the always-on control layout in order`() {
        val steps = blueprintControlTutorialSteps()
    ...
    fun `opening panel tutorial step resolves to the matching placement tool`() {
        val openingStep = blueprintControlTutorialSteps()
    ...
    fun `non-opening tutorial steps resolve to a neutral draw tool`() {
        val paramsStep = blueprintControlTutorialSteps()
            .first { it.target == BlueprintControlTutorialTarget.PARAMS_BUTTON }
        val helpStep = blueprintControlTutorialSteps()
    ...
    fun `canvas tutorial step prefers top pinned side anchored tooltip`() {
        val canvasStep = blueprintControlTutorialSteps()
